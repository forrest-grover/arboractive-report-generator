...
def test_section_head_has_no_right_side_subtitle() -> None:
    # Sections render only the left "01 - Foundation" title. No faded
    # right-side subtitle (`.num`) and no row-name listing as a section
    # subtitle ("pH · CEC · Organic Matter").
    html = render(make_report((DEMO_SAMPLE_A,)))
...
def test_summary_strip_is_absent() -> None:
    # The earlier "N priority gaps detected" alert + pH/CEC/OM KPI trio
    # was removed. Assert against multiple discriminators so any stray
    # remnant (HTML element, dead CSS class, body text) trips this.
    single_html = render(make_report((DEMO_SAMPLE_A,)))
    dual_html = render(make_report((DEMO_SAMPLE_A, DEMO_SAMPLE_B)))
    for html in (single_html, dual_html):
        assert 'class="summary"' not in html
        assert 'class="cell lead"' not in html
        assert "priority gap" not in html
        assert "Mostly healthy" not in html
        assert "Healthy soil profile" not in html
        # Dead CSS rule guards.
        assert ".summary" not in html
        assert ".summary .cell" not in html
        assert ".summary .lead" not in html


def test_subtitle_paragraph_is_absent() -> None:
    # The descriptive subtitle ("A whole-soil picture scored against
    # standard agronomic ranges...") was cut from the title row. Assert the
    # paragraph element and its CSS hook are both gone.
    single_html = render(make_report((DEMO_SAMPLE_A,)))
    dual_html = render(make_report((DEMO_SAMPLE_A, DEMO_SAMPLE_B)))
    for html in (single_html, dual_html):
        assert "A whole-soil picture" not in html
...
def test_pdf_single_sample_fits_one_page(tmp_path: Path) -> None:
    # The whole report is designed to land on a single US Letter page;
    # font/bar bumps were sized so the layout still fits. A second page
    # means something overflowed and we want the test to fail loudly.
    html = render(make_report((DEMO_SAMPLE_A,)))
    out = tmp_path / "single_pages.pdf"
    write_pdf(html, out)
    with pdfplumber.open(out) as pdf:
        assert len(pdf.pages) == 1


def test_pdf_two_samples_fits_one_page(tmp_path: Path) -> None:
    html = render(make_report((DEMO_SAMPLE_A, DEMO_SAMPLE_B)))
...
def test_pdf_contains_rendered_text_content(tmp_path: Path) -> None:
    ...
    # Dual mode hides title-row chips (col-band below labels the columns
    # instead), so sample names extract in their source case from the col-band.
